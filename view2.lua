...
--[[local ending = display.newText(" ", display.contentWidth*0.5, display.contentHeight*0.5)
	ending:setFillColor(0.5)
	ending.size = 200

	local result = composer.getVariable("complete")
	if result == 5  then
		ending.text = result .. "점 성공!!"
	else
		ending.text = result .. "점 실패..."
	end]]--

	local cat = display.newImageRect("image/cat.png", 150*1.2, 200*1.2)
	cat.x, cat.y = display.contentWidth*0.2, display.contentHeight*0.8
end
